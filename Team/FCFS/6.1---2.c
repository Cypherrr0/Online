...
int total_station;
int distance;
int target_output[100] ;
int clockwise_output[100];
int counterclockwise_output[100];
    ...
    while(strcmp(end,input)!=0)
    {
        ...
        {   
            ...
            {  
                ...
                //检查连续
                if(position==(target_num-1)*distance&&a!=0&&b!=0&&c!=0)//在站点有请求并且与头节点一致
                {   
                    ...
                    pCheck=p->next;count = 1;
                    //
                    while(count > 0)//当前已完成的请求目的和下一个请求目的相同
                    {
                        p = DeleteHead(p);
                        count--;
                    }
                ...
                else if(position!=(target_num-1)*distance)
                Output(position);
            }
            else //如果不在站点
            {
